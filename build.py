...
.chart-bar-label {{ width: 80px; font-size: 12px; text-align: right; color: var(--text-light); flex-shrink: 0; }}
.chart-bar-track {{ flex: 1; height: 24px; background: #eceff1; border-radius: 12px; position: relative; display: flex; align-items: center; }}
.chart-bar-fill {{
  height: 100%; border-radius: 12px; transition: width 0.5s ease; min-width: 0;
}}
.chart-bar-value {{
  position: relative; margin-left: 8px; font-size: 12px; font-weight: 600; color: var(--text); flex-shrink: 0;
}}
...
// ===== IndexedDB 備份儲存（防止 localStorage 被清除） =====
const DB_NAME = 'fri_backup';
const DB_VERSION = 1;
let idb = null;
...
// 從 localStorage 或 IndexedDB 還原資料
async function loadPersistedData() {{
  ...
  // 讀取 tasks
  const lsTasks = localStorage.getItem('fri_tasks');
  if (lsTasks && lsTasks !== '[]') {{
    tasks = JSON.parse(lsTasks);
  }} else {{
    const idbTasks = await idbGet('fri_tasks');
    if (idbTasks && idbTasks.length > 0) {{
      tasks = idbTasks;
      localStorage.setItem('fri_tasks', JSON.stringify(tasks));
      console.log('[資料還原] 從 IndexedDB 還原 ' + tasks.length + ' 筆交辦事項');
    }}
  }}

  // 讀取 users
  const lsUsers = localStorage.getItem('fri_users');
  if (lsUsers && lsUsers !== '{{}}') {{
    users = JSON.parse(lsUsers);
  }} else {{
    const idbUsers = await idbGet('fri_users');
    if (idbUsers && Object.keys(idbUsers).length > 0) {{
      users = idbUsers;
      localStorage.setItem('fri_users', JSON.stringify(users));
      console.log('[資料還原] 從 IndexedDB 還原使用者資料');
    }}
  }}
...
function doLogin() {{
  ...
  // Role hierarchy check: admin > manager > executor
  const roleLevel = {{ admin: 3, manager: 2, executor: 1 }};
  if (roleLevel[selectedRole] > roleLevel[user.role]) {{
    const needRole = ROLE_MAP[selectedRole].label;
    errEl.textContent = '此帳號無' + needRole + '權限，請聯繫管理員升級角色';
    return;
  }}

  // Login success - use selected role (may be lower than account role)
  currentUser = {{
    email: user.email,
    name: user.name,
    dept: user.dept || dept,
...
function getVisibleTasks() {{
  ...
  return tasks.filter(t =>
    // 自己建立的交辦事項
    (t.assignerEmail && t.assignerEmail.trim().toLowerCase() === myEmail) ||
    // 被指派的交辦事項
    t.assignees.some(a => a.email.trim().toLowerCase() === myEmail)
  );
}}
...
function showTaskDetail(taskId) {{
  ...
  const myEmail = currentUser.email.trim().toLowerCase();
  const isCreator = task.assignerEmail && task.assignerEmail.trim().toLowerCase() === myEmail;
  const canEdit = isAdmin() || (canManage() && isCreator);
...
function editTask(taskId) {{
  ...
  const myEmail = currentUser.email.trim().toLowerCase();
  const isCreator = task.assignerEmail && task.assignerEmail.trim().toLowerCase() === myEmail;
  if (!isAdmin() && !(canManage() && isCreator)) {{ showToast('您只能編輯自己建立的交辦事項', 'error'); return; }}
  closeModal();
...
function duplicateTask(taskId) {{
  if (!canManage()) {{ showToast('您沒有複製交辦事項的權限', 'error'); return; }}
  const task = tasks.find(t => t.id === taskId);
  if (!task) return;
  const nt = {{ ...JSON.parse(JSON.stringify(task)), id: generateId(), assignerEmail: currentUser.email, assigner: currentUser.name, status: 'pending', progress: 0, progressLog: [], createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }};
...
function deleteTask(taskId) {{
  ...
  const myEmail = currentUser.email.trim().toLowerCase();
  const isCreator = task.assignerEmail && task.assignerEmail.trim().toLowerCase() === myEmail;
  if (!isAdmin() && !(canManage() && isCreator)) {{ showToast('您只能刪除自己建立的交辦事項', 'error'); return; }}
...
function renderDashboard() {{
  ...
  document.getElementById('statusChart').innerHTML = Object.entries(sc).map(([k,v]) => '<div class="chart-bar"><span class="chart-bar-label">' + STATUS_MAP[k].label + '</span><div class="chart-bar-track"><div class="chart-bar-fill" style="width:' + (v/total*100) + '%;background:' + sColors[k] + '"></div><span class="chart-bar-value">' + v + '</span></div></div>').join('');
  ...
  document.getElementById('priorityChart').innerHTML = Object.entries(pc).map(([k,v]) => '<div class="chart-bar"><span class="chart-bar-label">' + PRIORITY_MAP[k].label + '</span><div class="chart-bar-track"><div class="chart-bar-fill" style="width:' + (v/total*100) + '%;background:' + pColors[k] + '"></div><span class="chart-bar-value">' + v + '</span></div></div>').join('');
